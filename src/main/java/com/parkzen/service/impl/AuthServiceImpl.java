...
import com.parkzen.repository.ParkingAreaRepository;
...
public class AuthServiceImpl implements AuthService {
    ...
    private final ParkingAreaRepository parkingAreaRepository;   // ← added
    ...
    public AuthResponse registerUser(UserRegisterRequest request) {
        ...
        return buildAuthResponse(token, user.getId(), user.getName(),
    ...
    public AuthResponse registerOwner(OwnerRegisterRequest request) {
        ...
        // Step 2 — Auto-create the linked ParkingArea in the same transaction.
        // Uses values sent from the registration form; falls back to safe defaults.
        double  pricePerHour        = request.getPricePerHour()         != null
                ? request.getPricePerHour()         : 50.0;
        boolean chargingAvailable   = request.getChargingAvailable()    != null
                ? request.getChargingAvailable()    : false;
        double  chargingPricePerHour = request.getChargingPricePerHour() != null
                ? request.getChargingPricePerHour() : 0.0;

        ParkingArea parkingArea = ParkingArea.builder()
                .name(request.getParkingAreaName())
                .address(request.getAddress())
                .latitude(request.getLatitude())
                .longitude(request.getLongitude())
                .totalSlots(0)           // owner adds slots from dashboard later
                .availableSlots(0)
                .pricePerHour(pricePerHour)
                .chargingAvailable(chargingAvailable)
                .chargingPricePerHour(chargingPricePerHour)
                .status("ACTIVE")
                .owner(owner)            // ← owner_id FK set here
                .build();

        parkingAreaRepository.save(parkingArea);
        log.info("ParkingArea '{}' created for owner: {}",
                parkingArea.getName(), owner.getEmail());

        String token = jwtUtil.generateToken(owner.getEmail(), owner.getRole().name());
        return buildAuthResponse(token, owner.getId(), owner.getName(),
                owner.getEmail(), owner.getRole().name());
    }
    ...
    public AuthResponse loginAdmin(LoginRequest request) {
        ...
        return buildAuthResponse(token, admin.getId(), "Admin",
                admin.getEmail(), admin.getRole().name());
    }

    // ── Shared builder ───────────────────────────────────────────────
    private AuthResponse buildAuthResponse(String token, Long id,
